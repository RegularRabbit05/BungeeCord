...
public class PlayerListItem extends DefinedPacket
{
    ...
    public void read(ByteBuf buf, ProtocolConstants.Direction direction, int protocolVersion)
    {
        if ( protocolVersion < ProtocolConstants.MINECRAFT_1_8 )
        {
            ...
            item.displayName = TextComponent.fromLegacy( item.username = readString( buf ) );
    ...
    public void write(ByteBuf buf, ProtocolConstants.Direction direction, int protocolVersion)
    {
        if ( protocolVersion < ProtocolConstants.MINECRAFT_1_8 )
        {
            ...
            writeString( item.getUsername(), buf ); // TODO: Server unique only!
            buf.writeBoolean( action != Action.REMOVE_PLAYER );
            buf.writeShort( item.ping == null ? 0 : item.ping );
        } else
        {
